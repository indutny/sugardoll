...
sugardoll.tokenizer = function tokenizer(code, plugins, options) {
  var tokens = [];
  ...
  if (plugins._preprocess) {
    tokens = plugins._preprocess(tokens);
  }
  ...
  for (var index = tokens.length - 1; index >= 0; index--) {
    ...
    if (token.type === 'name' &&
      ...
      // Caught invoke
      var plugin;
      if (token.value !== '_preprocess' && plugin = plugins[token.value]) {
        function collector() {
        ...
        if (collected !== false) {
          ...
          if (Array.isArray(code)) {
            // code is AST
            code = uglify.uglify.gen_code(code, options);
          }
...
sugardoll.parse = function parse(code, plugins, options) {
  // Wrap code in function, so `return` outside function won't
  // throw error
  code = 'function _() {' + code + '}';
  var ast = uglify.parser.parse(sugardoll.tokenizer(code, plugins, options));
  return getFnBody(ast);
}

sugardoll.compile = function compile(code, plugins, options) {
  var ast = sugardoll.parse(code, plugins, options);

  return uglify.uglify.gen_code(ast, options);
};

sugardoll.run = function run(code, plugins, options) {
